...
def theta(A):
    ...
    for i in range(5):
        for j in range(64):
            D[i][j]=(C[(i-1)%5][j])^(C[(i+1)%5][(j-1)%64])
    for i in range(5):
        for j in range(5):
            for k in range(64):
                A_out[i][j][k]=(A[i][j][k])^(D[i][k])
    print("after theta")
...
def generatetwofile():
    with open ('input_pattern.txt','w') as fi:
        with open('output_pattern.txt','w') as fo:
            
            for i in range(2):
                
                s=''.join(random.SystemRandom().choice(string.ascii_letters+string.digits) for _ in range(random.SystemRandom().randint(136, 272)))
                fi.write(s+'\n')
